...
static gboolean wakatime_init(GeanyPlugin *plugin, gpointer pdata)
{
    ...
    data->doc_open_id = g_signal_connect(geany_object, "document-open",
                                        G_CALLBACK(on_document_open), data);
    data->doc_save_id = g_signal_connect(geany_object, "document-save",
                                        G_CALLBACK(on_document_save), data);
    data->doc_activate_id = g_signal_connect(geany_object, "document-activate",
...
static void wakatime_cleanup(GeanyPlugin *plugin, gpointer pdata)
{
    ...
    if (data) {
        g_signal_handler_disconnect(geany_object, data->doc_open_id);
        g_signal_handler_disconnect(geany_object, data->doc_save_id);
        g_signal_handler_disconnect(geany_object, data->doc_activate_id);
        g_free(data);
    }
